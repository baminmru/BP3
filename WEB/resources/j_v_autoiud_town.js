...
var groupingFeature_autoiud_town = Ext.create('Ext.grid.feature.Grouping',{
groupByText:  'Группировать по этому полю',
showGroupsText:  'Показать группировку'
});
var interval_autoiud_town;
Ext.define('grid_autoiud_town', {
    extend:  'Ext.grid.Panel',
    alias: 'widget.g_v_autoiud_town',
    requires: [
        'Ext.grid.*',
        'Ext.form.field.Text',
    ...
    initComponent: function(){
        Ext.apply(this, {
        frame: false,
        store: store_v_autoiud_town,
        features: [groupingFeature_autoiud_town],
        ...
        columns: [
            {text: "Название", width:800, dataIndex: 'iud_town_name', sortable: true}
        ]
        ,
...
listeners: {render: function(e) {Ext.QuickTips.register({  target: e.getEl(), text: 'Название'});}}
    ...
    listeners: {
        ...
        ,
        	added:function(){
        			//interval_autoiud_town= setInterval(function() {  
        			//	store_v_autoiud_town.load();
        			//}, 60000);
        		}
        	,
        	destroy:function(){
        		//clearInterval(interval_autoiud_town);
        }
    },
    onDeleteConfirm:function(selection){
      if (selection) {
        Ext.Ajax.request({
            url:    rootURL+'index.php/c_v_autoiud_town/deleteRow',
            method:  'POST',
    		params: { 
    ...
    onDeleteClick: function(){
      ...
      if (selection) {
   	    if(CheckOperation('iud_t.edit')!=0 && OTAllowDelete('iud_t')){
        Ext.Msg.show({
            title:  'Удалить?',
    ...
    onAddClick: function(){
   	    if(CheckOperation('iud_t.edit')!=0 && OTAllowAdd('iud_t')){
            Ext.Ajax.request({
                url: rootURL+'index.php/c_v_autoiud_town/newRow',
                method:  'POST',
                params: { 
                },
                ...
                edit.prefix='c_v_autoiud_town';
                edit.setTitle('Создание документа:Города') ;
                var p=eval('iud_t_Panel_'+OTAddMode('iud_t')+'( res.data, false,null )') ;
                edit.add(p);
                edit.show();
                }
            });
    ...
    onEditClick: function(){
        var selection = this.getView().getSelectionModel().getSelection()[0];
        if (selection) {
   	    if(CheckOperation('iud_t.edit')!=0 ){
                var edit = Ext.create('iu.windowObjects');
                edit.prefix='c_v_autoiud_town';
                edit.setTitle('Редактирование документа: Города') ;
            var p=eval('iud_t_Panel_'+OTEditMode('iud_t')+'( selection.get(\'instanceid\'), false, selection )') ;
            edit.add(p);
            edit.show();
        }else{
        		Ext.MessageBox.show({
...
function iud_t_Jrnl(){ 
  ...
  var iud_t= Ext.create('Ext.form.Panel', {
       closable: true,
       id:     'iud_t_jrnl',
       title: 'Города',
      layout: 'fit',
      ...
      fieldDefaults: {
        ...
        items: [{
            itemId:'gr_autoiud_town',
            xtype:'g_v_autoiud_town',
            layout: 'fit',
            flex: 1,
            store: store_v_autoiud_town
    }] // tabpanel

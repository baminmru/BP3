...
class  M_iu_urok_docs extends CI_Model {
	...
	if (!empty($empId)){
	    $res = $this->jservice->get(array('Action' => 'GetRowData','FieldList'=>'B2G(iu_urok_docsid) as iu_urok_docsid, B2G(iu_urok_docsid) as id,B2G(instanceid) as instanceid, iu_urok_docs_BRIEF_F(iu_urok_docsid , NULL) as  brief,  DATE_FORMAT(adddate,\'%Y-%m-%d %H:%i:%s\') as  adddate,B2G(DocType) doctype, iud_doctype_BRIEF_F(doctype, NULL) as doctype_grid,info,activeversion, case activeversion  when -1 then \'Да\' when 0 then \'Нет\'End  as activeversion_grid,B2G(AddBy) addby, iu_u_def_BRIEF_F(addby, NULL) as addby_grid,version,fileref,fileref_ext', 'PartName' => 'iu_urok_docs', 'ID' =>  $empId 	));
	    if (!empty($res)) {
	        $result = $res[0];
	    }
	}
...
//////////////////////////// end support file data for: fileref
    ...
    function getRowsByInstance($id,$sort=array())
		{
	$res = $this->jservice->get(array('Action' => 'GetViewData','Sort'=>$sort,'FieldList'=>'B2G(iu_urok_docsid) as iu_urok_docsid, B2G(iu_urok_docsid) as id,B2G(instanceid) as instanceid, iu_urok_docs_BRIEF_F(iu_urok_docsid , NULL) as  brief,  DATE_FORMAT(adddate,\'%Y-%m-%d %H:%i:%s\') as  adddate,B2G(DocType) doctype, iud_doctype_BRIEF_F(doctype, NULL) as doctype_grid,info,activeversion, case activeversion  when -1 then \'Да\' when 0 then \'Нет\'End  as activeversion_grid,B2G(AddBy) addby, iu_u_def_BRIEF_F(addby, NULL) as addby_grid,version,fileref,fileref_ext', 'ViewName' => 'iu_urok_docs', 'WhereClause' => 'instanceid=G2B(\''. $id . '\')'));
	if (count($res) == 0) {
	    return null;
	} else {
	    return $res;
	...
	{
	$res = $this->jservice->get(array('Action' => 'GetViewData','Sort'=>$sort,'FieldList'=>'B2G(iu_urok_docsid) as iu_urok_docsid, B2G(iu_urok_docsid) as id,B2G(instanceid) as instanceid, iu_urok_docs_BRIEF_F(iu_urok_docsid , NULL) as  brief,  DATE_FORMAT(adddate,\'%Y-%m-%d %H:%i:%s\') as  adddate,B2G(DocType) doctype, iud_doctype_BRIEF_F(doctype, NULL) as doctype_grid,info,activeversion, case activeversion  when -1 then \'Да\' when 0 then \'Нет\'End  as activeversion_grid,B2G(AddBy) addby, iu_u_def_BRIEF_F(addby, NULL) as addby_grid,version,fileref,fileref_ext', 'ViewName' => 'iu_urok_docs', 'WhereClause' => ' parentstructrowid=G2B(\''. $id . '\')'));
	if (count($res) == 0) {
	    return null;
	} else {
	    return $res;

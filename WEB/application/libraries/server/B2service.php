...
class B2service
{
	...
	public function GetRoleData()
    {
        ...
        $res = $this->db->query("select b2g(roles_operationsid) opid, roles_def.name rolename,
								b2g(roles_def.instanceid) id,
								allobjects,colegsobject,substructobjects,
								roles_operations.name opname,	allowaction
								from roles_def join roles_operations
								on roles_def.instanceid=roles_operations.instanceid
								order by roles_def.name,roles_operations.name"
								);
								$rolename='';
        while ($row = $res->fetch_assoc()){
		    if( $rolename != $row['rolename']){
				...
				$myrow->allobjects=($row['allobjects']==-1);
				$myrow->colegsobject=($row['colegsobject']==-1);
				$myrow->substructobjects=($row['substructobjects']==-1);
			}
			$fld = str_replace('.','__',$row['opname']);
            $myrow->$fld=($row['allowaction']==-1);
        }
        $res->close();
		$result[] = $myrow;
        return $result;
  ...
  /*    
	...
	public function ProcessRole()
    {
			...
			file_put_contents($this->config['logpath'].'/_debug.txt',
				'
				>>>: '.'Process role:'.$id .'  '.$caption.'', FILE_APPEND);
			...
			if ($id==''){
				...
				$stmt->close();
				$rowid= '{'.strtoupper($result).'}';
				
				$typename =  'ROLES';
				
				$stmt = $this->db->prepare('CALL Instance_Save(?,?,?,?)');
				$stmt->bind_param('ssss', $this->SessionData['SessionID'], $id, $typename, $caption);
				$res = $stmt->execute();
				...
				if (!$res){
					...
					if ($this->config['log']==true)		
						file_put_contents($this->config['logpath'].'/_debug.txt',			'
						>>>: '.'Error while creating document: '.$stmt->error.'\r\n'			, FILE_APPEND);
				}
				$stmt->close();	
				
	
	
				$stmt = $this->db->prepare('CALL roles_def_Save(?,?,?,?,?,0,0,0,0,0,0,0)');
				$stmt->bind_param('sssss', $this->SessionData['SessionID'], $id, $rowid, $caption,$caption);
				$res = $stmt->execute();
				...
				if (!$res){
					...
					if ($this->config['log']==true)		
						file_put_contents($this->config['logpath'].'/_debug.txt',			'
						>>>: '.'Error saving role header: '.$stmt->error .'\r\n'			, FILE_APPEND);
			...
			foreach ($values as $key => $value){
				...
				switch ($key) {
				   ...
				   case 'instanceid':
						 ...
						 break;
				   case 'rolename':
						 $qry = "update roles_def set name='".$caption."', thecomment='".$caption."' where instanceid=g2b('". $id . "')";
						 ...
						 if ($this->config['log']==true){		
							file_put_contents($this->config['logpath'].'/_debug.txt',
								'
								>>>: '.'Process role:'.$id .' -> '.$qry.' '.$stmt->error, FILE_APPEND);
						}
						$stmt->close();
						break;
						 
				case 'colegsobject':
				case 'substructobjects':
				case 'allobjects' :
						
						 $qry = "update roles_def set ".$key."=".$bvalue." where instanceid=g2b('". $id . "')";
						 $stmt = $this->db->prepare($qry);
						 $res = $stmt->execute();
						 ...
						 break;
				default:
						 $qry = "call updateRoles(".$bvalue.",'".str_replace('__','.',$key)."','". $id . "')";
						  $stmt = $this->db->prepare($qry);
						 $res = $stmt->execute();
			 ...
			 if ($this->config['log']==true){		
				file_put_contents($this->config['logpath'].'/_debug.txt',
					'
					>>>: Process role:'.$id .' -> return OK', FILE_APPEND);
			}
			return 'OK';
	...
	function treeNodes($mass, $first, $parentId)
	{
		...
		*/
		...
		public function GetCommentTree($instanceid,$parentid)
		{
		
			$query="select b2g(instanceid) instanceid,  b2g(iu_cm_msgid) id ,b2g(parentrowid) parentid,`iu_u_def_brief_f`(`iu_cm_msg`.`theauthor`,NULL)  `theauthor`,thedate,info
				from iu_cm_msg  where parentrowid=g2b('".$parentid."') and instanceid= g2b('".$instanceid."') order by thedate";
			...
			{
				$children=$this->GetCommentTree( $tree[$i]['instanceid'], $tree[$i]['id']);
		...
		{
		    $instanceid = isset($this->request['instanceid']) ? $this->request['instanceid'] : ''; 
			$query="select instanceid,	instanceid id ,'THEME' parentid, iu_cm_def_theauthor theauthor,iu_cm_def_thedate thedate  ,iu_cm_def_thetheme info
					from v_autoiu_cm_def  where instance_archived=0  and  iu_cm_def_theprocess_id='".$instanceid."'
					and iu_cm_def_isdiscussion_val=-1  and iu_cm_def_thevideo_id is null and iu_cm_def_thedoc_id is null
					order by iu_cm_def_thedate";
			...
			for($i=0; $i<count($tree); $i++) 
			{
				$children =$this->GetCommentTree($tree[$i]['id'],'{00000000-0000-0000-0000-000000000000}');
				if($children){
					$tree[$i]['children'] = $children;
					$tree[$i]['expanded'] = true;
					$tree[$i]['leaf'] = false;
		...
		public function GetCommentVideoThemes()
		{
			...
			$children =$this->GetCommentTree($instanceid,'{00000000-0000-0000-0000-000000000000}');

...
Public Class ExtJSMakerMYSQL
    ...
    Private Sub ExtJSMaker_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Next
            ...
            Dim dt As DataTable
        dt = Manager.Session.GetData("select " + Manager.Session.GetProvider.InstanceFieldList + " from instance where objtype ='bp3list' order by name")
    ...
    Private Function SysMake_Include() As String
        ...
        Dim JJJ As bp3list.bp3list.Application
        ...
        Dim om As bp3card.bp3card.bp3card_def
        ...
        For i = 0 To chkObjType.CheckedItems.Count - 1
            ...
            Dim dt_crd As DataTable
            Dim card_app As bp3card.bp3card.Application
            dt_crd = Manager.Session.GetData("select " + Manager.Session.GetProvider().ID2Base("instanceid", "instanceid") + " from bp3card_def where cardfor= " + Manager.Session.GetProvider().ID2Const(ot.ID))


            For m = 0 To dt_crd.Rows.Count - 1
                card_app = Manager.GetInstanceObject(New Guid(dt_crd.Rows(m)("instanceid").ToString()))

                om = card_app.bp3card_def.Item(1)
                mode = om.Name

                sw.Append(vbCrLf & "<!--   type:" & ot.Name & " mode:" & mode & " -->" & vbCrLf)
                For j = 1 To AppForDoc(ot).bp3doc_store.Count
                    P = AppForDoc(ot).bp3doc_store.Item(j)

                    If PartIsPresent(P, card_app) Then
            ...
            For j = 1 To AppForDoc(ot).bp3doc_store.Count
                ...
                If PartIsPresent(P, Nothing) Then
                    sw.Append(vbCrLf & "     <script type='text/javascript' src='resources/" & P.Name.ToLower() & "_" & mode & ".js'></script>")
                End If
        ...
        For i = 0 To chkJournals.CheckedItems.Count - 1
            ...
            If Not JJJ Is Nothing Then
                Try
                    Dim va As String
                    Dim qry As bp3qry.bp3qry.bp3qry_def
                    qry = JJJ.bp3list_def.Item(1).SourceView
                    va = qry.the_Alias
    ...
    Private Function SysMake_Menu() As String
        ...
        Dim p As bp3list.bp3list.bp3list_def


        Dim ot As bp3doc.bp3doc.bp3doc_def
        Dim prt As bp3doc.bp3doc.bp3doc_store

        Dim qry As bp3qry.bp3qry.Application
        Dim pv As bp3qry.bp3qry.bp3qry_def
        ...
        For i = 0 To chkJournals.CheckedItems.Count - 1
            ...
            If Not JJJ Is Nothing Then
                ...
                p = JJJ.bp3list_def.Item(1)
                pv = p.SourceView
                qry = pv.Application
                prt = pv.SrcPart
                ot = TypeForStruct(prt)
                ...
                If Not p Is Nothing Then
                    ...
                    sw.Append(vbCrLf & "    text:               '" & ot.TheCaption & "',")
    ...
    Private Sub PartMake_CI(ByVal pcol As bp3doc.bp3doc.bp3doc_store_col)
        Dim xout As String
        ...
        For i = 1 To pcol.Count
            P = pcol.Item(i)
            If P.PartType <> bp3doc.bp3doc.enumPartType.PartType_Rassirenie Then
    ...
    'Private Function TypeMartImport(ByVal OT As bp3doc.bp3doc.bp3doc_def) As String
    '    Dim os As bp3doc.bp3doc.bp3doc_store = Nothing
    '    Dim osRoot As bp3doc.bp3doc.bp3doc_store = Nothing
    ...
    '    For i = 1 To OT.bp3doc_store.Count
    '        os = OT.bp3doc_store.Item(i)
    '        If os.PartType = bp3doc.bp3doc.enumPartType.PartType_Kollekciy And os.Name.ToLower.Contains("_photo") Then
    ...
    Private Function GetIconName(ByVal P As bp3doc.bp3doc.bp3doc_store, ByVal tabname As String) As String
        Dim i As Integer
    ...
    Private Sub TypeMaker(doc_app As bp3doc.bp3doc.Application, card_app As bp3card.bp3card.Application)
        ...
        Dim mainPartObj As bp3doc.bp3doc.bp3doc_store = Nothing
        Dim tabcol As Collection
        Dim tn As String
        Dim ii As Integer
        Dim objmode As bp3card.bp3card.Application
        Dim modename As String
        If card_app Is Nothing Then
            modename = ""
        Else
            modename = card_app.bp3card_def.Item(1).Name
        End If
        ...
        doc_app.bp3doc_store.Sort = "sequence"
        PartMake_CI(doc_app.bp3doc_store)
        For i = 1 To doc_app.bp3doc_store.Count
            P = doc_app.bp3doc_store.Item(i)

            If PartIsPresent(P, card_app) Then
                If P.PartType <> bp3doc.bp3doc.enumPartType.PartType_Rassirenie Then

                    If mainPartName = "" And P.PartType = bp3doc.bp3doc.enumPartType.PartType_Stroka Then
                        mainPartName = "int_" & P.Name.ToLower & "_" & modename
                    ...
                    If mainPart = P.Name.ToLower Then
                        sw.Append(vbCrLf & PartMake_InterfaceJS(ot, P, objmode, True))
                    Else
                        sw.Append(vbCrLf & PartMake_InterfaceJS(ot, P, objmode, False))
                    End If


                    sw.Append(vbCrLf & "function DefineForms_" & P.Name.ToLower() & "_" & modename & "(){" & vbCrLf)
                    sw.Append(vbCrLf & PartMake_FormPanelJS(ot, P, objmode))
                    sw.Append(vbCrLf & "}")
                    Tool_WriteFile(sw.ToString(), textBoxOutPutFolder.Text & "_js\", P.Name.ToLower() & "_" & modename & ".js")
                End If
            End If
        Next
        ...
        sw.Append(vbCrLf & "  " & ot.Name.ToLower() & "_" & modename & "= null;")
        sw.Append(vbCrLf & "function " & ot.Name & "_Panel_" & modename & "(objectID, RootPanel, selection){ " & vbCrLf)
        ...
        For i = 1 To doc_app.bp3doc_store.Count
            P = doc_app.bp3doc_store.Item(i)
            If PartIsPresent(P, objmode) Then
                If P.PartType <> bp3doc.bp3doc.enumPartType.PartType_Rassirenie Then
                    sw.Append(vbCrLf & PartMake_JSStore(P))
                End If
            End If
        Next
        ...
        For i = 1 To doc_app.bp3doc_store.Count
            ...
            If PartIsPresent(P, objmode) Then
                If P.PartType <> bp3doc.bp3doc.enumPartType.PartType_Rassirenie Then
                    sw.Append(vbCrLf & "          DefineForms_" & P.Name.ToLower & "_" & modename & "();")
                End If
            End If
        Next
            ...
            If PartIsPresent(P, objmode) Then
                If P.PartType <> bp3doc.bp3doc.enumPartType.PartType_Rassirenie Then
                    ...
                    If P.PartType = bp3doc.bp3doc.enumPartType.PartType_Derevo Then
                        sw.Append(vbCrLf & "  var   int_" & P.Name.ToLower & "_" & modename & "     = DefineInterface_" & P.Name.ToLower & "_" & modename & "('int_" & P.Name.ToLower & "', treestore_" & P.Name.ToLower() & ");")
                    Else
                        ...
                        If P.PartType = bp3doc.bp3doc.enumPartType.PartType_Stroka Then
                            ...
                            For ii = 1 To P.bp3doc_field.Count
                                If IsFieldPresent(P, P.bp3doc_field.Item(ii).ID, objmode) And Not P.bp3doc_field.Item(ii).TheStyle.Contains("hidden") Then
                                    If P.bp3doc_field.Item(ii).TabName <> "" Then
                                        tn = P.bp3doc_field.Item(ii).TabName
                                        Try
                                            tabcol.Add(tn, tn)
                                        Catch ex As Exception
                                            ' non unique
                            ...
                            Next
                            If mainPart = P.Name.ToLower Then
                                sw.Append(vbCrLf & "     var   int_" & P.Name.ToLower & "_" & modename & "     =      DefineInterface_" & P.Name.ToLower & "_" & modename & "('int_" & P.Name.ToLower & "',store_" & P.Name.ToLower & ", selection);")
                            Else
                                sw.Append(vbCrLf & "     var   int_" & P.Name.ToLower & "_" & modename & "     =      DefineInterface_" & P.Name.ToLower & "_" & modename & "('int_" & P.Name.ToLower & "',store_" & P.Name.ToLower & ");")
                            End If
                            If tabcol.Count > 0 Then
                                For ii = 1 To tabcol.Count
                                    sw.Append(vbCrLf & "     var   int_" & P.Name.ToLower & "_" & modename & "_tab" & ii.ToString() & "     =      DefineInterface_" & P.Name.ToLower & "_" & modename & "_tab" & ii.ToString() & "('int_" & P.Name.ToLower & "_tab" & ii.ToString() & "',store_" & P.Name.ToLower & ");")
                                Next
                ...
                Else
                    If P.PartType = bp3doc.bp3doc.enumPartType.PartType_Derevo Then
                        sw.Append(vbCrLf & "  var   int_" & P.Name.ToLower & "_" & modename & "     = DefineInterface_" & P.Name.ToLower & "_" & modename & "('int_" & P.Name.ToLower & "', treestore_mart_" & P.Name.ToLower() & ");")
                    Else
                        sw.Append(vbCrLf & "     var   int_" & P.Name.ToLower & "_" & modename & "     =      DefineInterface_" & P.Name.ToLower & "_" & modename & "('int_" & P.Name.ToLower & "',store_mart_" & P.Name.ToLower & ");")
        ...
        If doc_app.bp3doc_store.Count > 10 Then
        ...
        sw.Append(vbCrLf & "     " & ot.Name.ToLower() & "_" & modename & "= Ext.create('Ext.form.Panel', {")
        ...
        If ot.CommitFullObject = bp3doc.bp3doc.enumBoolean.Boolean_Da Then
            ...
            sw.Append(vbCrLf & "        						}else{")
            sw.Append(vbCrLf & "        							if(typeof(" + ot.Name.ToLower() + "_" & modename & ".onButtonCancel) == 'function') " + ot.Name.ToLower() + "_" & modename & ".onButtonCancel();")
            'sw.Append(vbCrLf & "        							Ext.Ajax.request(")
            'sw.Append(vbCrLf & "        								{")
        ...
        If doc_app.bp3doc_store.Count > 10 Then
            ...
            sw.Append(vbCrLf & "								var tabpanel=" & ot.Name.ToLower & "_" & modename & ".getComponent('tabs_" & ot.Name.ToLower() + "');")
        ...
        For i = 1 To doc_app.bp3doc_store.Count
            ...
            If PartIsPresent(P, objmode) Then
                ...
                ElseIf P.PartType = bp3doc.bp3doc.enumPartType.PartType_Stroka Then
                    ...
                    'End If
                    sw.Append(vbCrLf & "            items:[ // panel on tab ")
                    sw.Append(vbCrLf & "int_" & P.Name.ToLower() & "_" & modename & "")
                    ...
                    If tabcol.Count > 0 Then
                        For ii = 1 To tabcol.Count
                            ...
                            'End If
                            sw.Append(vbCrLf & "            items:[ // panel on tab ")
                            sw.Append(vbCrLf & "int_" & P.Name.ToLower() & "_" & modename & "_tab" & ii.ToString())
                        Next
                ...
                ElseIf P.PartType = bp3doc.bp3doc.enumPartType.PartType_Derevo Then
                    sw.Append(vbCrLf & "            { // begin part tab")
                    sw.Append(vbCrLf & "            xtype:'panel',")
                    sw.Append(vbCrLf & "            border:0,")
                    sw.Append(vbCrLf & "            title: '" & P.Caption & "',")
        ...
        sw.Append(vbCrLf & "       " & ot.Name.ToLower() & "_" & modename & ".closable= true;")
        sw.Append(vbCrLf & "       " & ot.Name.ToLower() & "_" & modename & ".title= '" & ot.TheCaption & "';")
        sw.Append(vbCrLf & "       " & ot.Name.ToLower() & "_" & modename & ".frame= true;")

        sw.Append(vbCrLf & "    }else{")

        sw.Append(vbCrLf & "       " & ot.Name.ToLower() & "_" & modename & ".closable= false;")
        sw.Append(vbCrLf & "       " & ot.Name.ToLower() & "_" & modename & ".title= '';")
        sw.Append(vbCrLf & "       " & ot.Name.ToLower() & "_" & modename & ".frame= false;")
            ...
            If PartIsPresent(P, objmode) Then
                If P.PartType <> bp3doc.bp3doc.enumPartType.PartType_Rassirenie Then
                    ...
                    If P.PartType = bp3doc.bp3doc.enumPartType.PartType_Stroka Then
                        ...
                        sw.Append(vbCrLf & "   int_" & P.Name.ToLower() & "_" & modename & ".setActiveRecord(record,objectID);	")
                        ...
                        If tabcol.Count > 0 Then
                            For ii = 1 To tabcol.Count
                                sw.Append(vbCrLf & "   int_" & P.Name.ToLower() & "_" & modename & "_tab" & ii.ToString & ".setActiveRecord(record,objectID);	")
                            Next
                        End If
                    ...
                    ElseIf P.PartType = bp3doc.bp3doc.enumPartType.PartType_Derevo Then
                        sw.Append(vbCrLf & "   int_" & P.Name.ToLower() & "_" & modename & ".instanceid=objectID;	")
                    Else
                        sw.Append(vbCrLf & "   int_" & P.Name.ToLower() & "_" & modename & ".instanceid=objectID;	")
                        sw.Append(vbCrLf & "       store_" & P.Name.ToLower() & ".load(  {params:{ instanceid:objectID} } );")
                    End If
        ...
        sw.Append(vbCrLf & "    return " & ot.Name.ToLower() & "_" & modename & ";")
        sw.Append(vbCrLf & "}")

        Tool_WriteFile(sw.ToString(), textBoxOutPutFolder.Text & "_js\", ot.Name.ToLower() & "_" & modename & ".js")
    ...
    Private Function SysMake_enumStore() As String
        ...
        Dim dt_ot As DataTable
        Dim doc_app As bp3ft.bp3ft.Application
        dt_ot = Manager.Session.GetData("select " + Manager.Session.GetProvider().ID2Base("instanceid", "instanceid") + " from bp3ft_def order by name ")
        ...
        For i = 0 To dt_ot.Rows.Count - 1
            ...
            If ft.TypeStyle = bp3doc.bp3doc.enumTypeStyle.TypeStyle_Perecislenie Then
                ...
                AppForFT(ft).bp3ft_enums.Sort = "NameValue"
                For j = 1 To doc_app.bp3ft_enums.Count
                    If j > 1 Then
                        sw.Append(vbCrLf & ",")
                    End If
                    sw.Append("[")
                    sw.Append("'" & AppForFT(ft).bp3ft_enums.Item(j).Name & "',")
                    sw.Append(AppForFT(ft).bp3ft_enums.Item(j).NameValue)
                    sw.Append("]")
                Next
    ...
    Private Function PartMake_JSStore(ByVal P As bp3doc.bp3doc.bp3doc_store) As String

        If P.PartType = bp3doc.bp3doc.enumPartType.PartType_Rassirenie Then Return ""
        ...
        If TypeName(P.Parent.Parent) = "Application" Then
            isroot = True
        End If
        ...
        End If
        Dim i As Integer
        For i = 1 To P.bp3doc_store.Count
            sw.Append(vbCrLf & PartMake_JSStore(P.bp3doc_store.Item(i)))
        Next

        Return sw.ToString()
    End Function

    Private Function PartMake_JSModelAndStore(ByVal P As bp3doc.bp3doc.bp3doc_store) As String
        If P.PartType = bp3doc.bp3doc.enumPartType.PartType_Rassirenie Then Return ""
        Dim sw As StringBuilder
        sw = New StringBuilder()
        ...
        For i = 1 To P.bp3doc_field.Count
            fld = P.bp3doc_field.Item(i)
            ft = fld.fieldType
            If ft.TypeStyle = bp3doc.bp3doc.enumTypeStyle.TypeStyle_Skalyrniy_tip Then
    ...
    Private Function PartMake_ComboStore(ByVal P As bp3doc.bp3doc.bp3doc_store) As String
        If P.PartType = bp3doc.bp3doc.enumPartType.PartType_Rassirenie Then Return ""
        Dim sw As StringBuilder
        sw = New StringBuilder()
    ...
    'Private Function PartMake_XML(ByVal ot As bp3doc.bp3doc.bp3doc_def, ByVal P As bp3doc.bp3doc.bp3doc_store) As String
    '    Dim sout As String = ""
    '    Dim fld As bp3doc.bp3doc.bp3doc_field
    ...
    '        For j = 1 To P.bp3doc_field.Count
    '            fld = P.bp3doc_field.Item(j)
    ...
    Private Function PartMake_CIModel(ByVal p As bp3doc.bp3doc.bp3doc_store) As String
        If p.PartType = bp3doc.bp3doc.enumPartType.PartType_Rassirenie Then Return ""
        Dim ot As bp3doc.bp3doc.bp3doc_def
        ...
        For j = 1 To p.bp3doc_field.Count
            ...
            If ft.TypeStyle <> bp3doc.bp3doc.enumTypeStyle.TypeStyle_Element_oformleniy Then
                ...
                If ft.TypeStyle = bp3doc.bp3doc.enumTypeStyle.TypeStyle_Perecislenie Then
                    ...
                    For i = 1 To AppForft(ft).bp3ft_enums.Count
                        flist = flist & " when " & AppForFT(ft).bp3ft_enums.Item(i).NameValue.ToString() & " then \'" & AppForFT(ft).bp3ft_enums.Item(i).Name & "\'"
                    Next
                    flist = flist & " else \'\' "
                    flist = flist & " end  "
                ...
                If ft.TypeStyle = bp3doc.bp3doc.enumTypeStyle.TypeStyle_Ssilka Then
                    ...
                    If ft.Name.ToLower() = "multiref" Then
                        If fld.ReferenceType = bp3doc.bp3doc.enumReferenceType.ReferenceType_Na_stroku_razdela Then
                            flist = flist & "," & fld.Name.ToLower
                            flist = flist & "," & " " & CType(fld.RefToPart, bp3doc.bp3doc.bp3doc_store).Name & "_MREF_F(" & fld.Name.ToLower() & ", NULL)"
                            flist = flist & " as  " & fld.Name.ToLower() & "_grid"

                        End If
                        If fld.ReferenceType = bp3doc.bp3doc.enumReferenceType.ReferenceType_Na_ob_ekt_ Then
                    ...
                    Else
                        If fld.ReferenceType = bp3doc.bp3doc.enumReferenceType.ReferenceType_Na_stroku_razdela Then
                            flist = flist & ",B2G(" & fld.Name & ") " & fld.Name.ToLower
                            'flist = flist & "," & " " & CType(fld.RefToPart, bp3doc.bp3doc.bp3doc_store).Name & "_BRIEF_F(B2G(" & fld.Name.ToLower() & "), NULL)"
                            flist = flist & "," & " " & CType(fld.RefToPart, bp3doc.bp3doc.bp3doc_store).Name & "_BRIEF_F(" & fld.Name.ToLower() & ", NULL)"
                            flist = flist & " as " & fld.Name.ToLower() & "_grid"
                        End If
                        If fld.ReferenceType = bp3doc.bp3doc.enumReferenceType.ReferenceType_Na_ob_ekt_ Then
                            flist = flist & ",B2G(" & fld.Name & ") " & fld.Name.ToLower
        ...
        If p.UseArchiving = bp3doc.bp3doc.enumBoolean.Boolean_Da Then
    ...
    'Private Function PartMake_CIMartModel(ByVal p As bp3doc.bp3doc.bp3doc_store) As String
    '    If p.PartType = bp3doc.bp3doc.enumPartType.PartType_Rassirenie Then Return ""
    '    Dim isroot As Boolean = False
    '    If TypeName(p.Parent.Parent) = "Application" Then
    '        isroot = True
    ...
    '                flist = flist & "," & " case " & fld.Name.ToLower() & " "
    '                For i = 1 To ft.bp3ft_enums.Count
    '                    flist = flist & " when " & ft.bp3ft_enums.Item(i).NameValue.ToString() & " then \'" & ft.bp3ft_enums.Item(i).Name & "\'"
    '                Next
    '                flist = flist & "End "
    '                flist = flist & " as " & fld.Name.ToLower() & "_grid"
    ...
    Private Function PartMake_CIController(ByVal p As bp3doc.bp3doc.bp3doc_store) As String
        If p.PartType = bp3doc.bp3doc.enumPartType.PartType_Rassirenie Then Return ""
        Dim ot As bp3doc.bp3doc.bp3doc_def
        ...
        Dim sortfld As bp3doc.bp3doc.bp3doc_field = Nothing
        For j = 1 To p.bp3doc_field.Count
            fld = p.bp3doc_field.Item(j)
            ft = fld.fieldType
            If ft.TypeStyle <> bp3doc.bp3doc.enumTypeStyle.TypeStyle_Element_oformleniy Then
        ...
        For j = 1 To p.bp3doc_field.Count
            ...
            ft = fld.fieldType
            If ft.TypeStyle <> bp3doc.bp3doc.enumTypeStyle.TypeStyle_Element_oformleniy Then
                If fld.IsBrief = bp3doc.bp3doc.enumBoolean.Boolean_Da And sortfld Is Nothing Then
                    sortfld = fld
                    Exit For
                End If
            End If
        ...
        If ot.CommitFullObject = bp3doc.bp3doc.enumBoolean.Boolean_Da Then
            ...
            Dim sft As bp3ft.bp3ft.bp3ft_def
            sft = sortfld.fieldType
            If sft.TypeStyle = bp3doc.bp3doc.enumTypeStyle.TypeStyle_Ssilka Then
    ...
    'Private Function PartMake_CIMartController(ByVal p As bp3doc.bp3doc.bp3doc_store) As String
    '    If p.PartType = bp3doc.bp3doc.enumPartType.PartType_Rassirenie Then Return ""
    ...
    Private Function PartMake_GridJS(ByVal ot As bp3doc.bp3doc.bp3doc_def, ByVal P As bp3doc.bp3doc.bp3doc_store, ByVal objmode As bp3card.bp3card.Application) As String
        Dim sw As StringBuilder
        sw = New StringBuilder()
        ...
        Dim i As Integer
        Dim modename As String
        If objmode Is Nothing Then
            modename = ""
        Else
            modename = objmode.bp3card_def.Item(1).Name
        End If
        ...
        sw.Append(vbCrLf & "   	if(CheckOperation('" + ot.Name + ".edit')!=0){")
        sw.Append(vbCrLf & "                var edit = Ext.create('EditWindow_" & P.Name.ToLower() & modename & "');")
        ...
        sw.Append(vbCrLf & "       stateId:  '" & P.Name.ToLower() & modename & "',")
        ...
        If Not GetPartRestriction(P, objmode) Is Nothing Then
            If GetPartRestriction(P, objmode).AllowAdd = bp3doc.bp3doc.enumBoolean.Boolean_Net Then
        ...
        If Not GetPartRestriction(P, objmode) Is Nothing Then
            If GetPartRestriction(P, objmode).AllowEdit = bp3doc.bp3doc.enumBoolean.Boolean_Net Then
                sw.Append(vbCrLf & "                hidden : true,")
            End If
        End If
        ...
        sw.Append(vbCrLf & "                    scope:  this,")
        If Not GetPartRestriction(P, objmode) Is Nothing Then
            If GetPartRestriction(P, objmode).AllowDelete = bp3doc.bp3doc.enumBoolean.Boolean_Net Then
                sw.Append(vbCrLf & "                   hidden : true,")
            End If
        End If
        ...
        For i = 1 To P.bp3doc_field.Count
            fld = P.bp3doc_field.Item(i)
            If IsFieldPresent(P, fld.ID, objmode) And Not fld.TheStyle.Contains("hidden") Then
    ...
    Private Function PartMake_GridGridJS(ByVal ot As bp3doc.bp3doc.bp3doc_def, ByVal P As bp3doc.bp3doc.bp3doc_store, ByVal objmode As bp3card.bp3card.Application) As String
        Dim sw As StringBuilder
        sw = New StringBuilder()
        ...
        Else
            modename = objmode.bp3card_def.Item(1).Name
        End If
        Dim child As bp3doc.bp3doc.bp3doc_store
        child = P.bp3doc_store.Item(1)

        sw.Append(PartMake_GridChildJS(ot, child, objmode))
    ...
    Private Function PartMake_GridChildJS(ByVal ot As bp3doc.bp3doc.bp3doc_def, ByVal P As bp3doc.bp3doc.bp3doc_store, ByVal objmode As bp3card.bp3card.Application, Optional ByVal bbar As Boolean = True) As String
        Dim sw As StringBuilder
        sw = New StringBuilder()
    ...
    Private Function PartMake_TreeJS(ByVal ot As bp3doc.bp3doc.bp3doc_def, ByVal P As bp3doc.bp3doc.bp3doc_store, ByVal objmode As bp3card.bp3card.Application) As String
        Dim sw As StringBuilder
        sw = New StringBuilder()
    ...
    Private Function PartMake_TreeGridJS(ByVal ot As bp3doc.bp3doc.bp3doc_def, ByVal P As bp3doc.bp3doc.bp3doc_store, ByVal objmode As bp3card.bp3card.Application) As String
        Dim sw As StringBuilder
        sw = New StringBuilder()
        ...
        sw.Append(PartMake_GridChildJS(ot, child, objmode, False))
    ...
    Private Function PartMake_InterfaceJS(ByVal ot As bp3doc.bp3doc.bp3doc_def, ByVal P As bp3doc.bp3doc.bp3doc_store, ByVal objmode As bp3card.bp3card.Application, isMain As Boolean) As String

        If P.PartType = bp3doc.bp3doc.enumPartType.PartType_Rassirenie Then Return ""

        Dim itype As InterfaceType
        Dim s As String
        Dim modename As String
        If objmode Is Nothing Then
            modename = ""
        ...
        If isMain Then
            s = vbCrLf & "function DefineInterface_" & P.Name.ToLower() & "_" & modename & "(id,mystore,selection){" & vbCrLf
            itype = AnalyzeInterfaceForGUI(P, Nothing)
        Else
            s = vbCrLf & "function DefineInterface_" & P.Name.ToLower() & "_" & modename & "(id,mystore){" & vbCrLf
            itype = AnalyzeInterfaceForGUI(P, objmode)
        End If

        Dim t As String
        t = vbCrLf & "function DefineInterface_" & P.Name.ToLower() & "_" & modename & "(id,treestore_" & P.Name.ToLower() & "){" & vbCrLf


        Select Case itype
            Case InterfaceType.InterfaceTypePanel
                ...
                For i = 1 To P.bp3doc_field.Count
                    If IsFieldPresent(P, P.bp3doc_field.Item(i).ID, objmode) And Not P.bp3doc_field.Item(i).TheStyle.Contains("hidden") Then
                        If P.bp3doc_field.Item(i).TabName <> "" Then
                            tn = P.bp3doc_field.Item(i).TabName
                            Try
                                tabcol.Add(tn, tn)
                            Catch ex As Exception
                                ' non unique
                ...
                Next
                If tabcol.Count = 0 Then
                    Return s & PartMake_PlaneJS(ot, P, objmode, isMain) & vbCrLf & "};"
                Else
                    Dim s1 As String
                    s1 = s & PartMake_PlaneJS(ot, P, objmode, isMain) & vbCrLf & "};"
                    For i = 1 To tabcol.Count
                        s = vbCrLf & "function DefineInterface_" & P.Name.ToLower() & "_" & modename & "_tab" & i.ToString & "(id,mystore){" & vbCrLf
                        s1 = s1 & vbCrLf & s & PartMake_PlaneJS(ot, P, objmode, isMain, tabcol.Item(i), "_tab" & i.ToString) & vbCrLf & "};"
                    Next
                    Return s1
                End If
            ...
            Case InterfaceType.InterfaceTypeGrid
                Return s & PartMake_GridJS(ot, P, objmode) & vbCrLf & "};"

            Case InterfaceType.InterfaceTypeTree
                Return t & PartMake_TreeJS(ot, P, objmode) & vbCrLf & "};"
            Case InterfaceType.InterfaceTypeTreeGrid
                Return t & PartMake_TreeGridJS(ot, P, objmode) & vbCrLf & "};"
            Case InterfaceType.InterfaceTypeGridGrid
                Return s & PartMake_GridGridJS(ot, P, objmode) & vbCrLf & "};"
            Case InterfaceType.InterfaceTypeCommon
                Return s & PartMake_GridJS(ot, P, objmode) & vbCrLf & "};"
    ...
    Private Function PartMake_GenStores(ByVal pcol As bp3doc.bp3doc.bp3doc_store_col) As String
        Dim sw As StringBuilder
        sw = New StringBuilder()
        Dim P As bp3doc.bp3doc.bp3doc_store
        ...
        For i = 1 To pcol.Count
            ...
            sw.Append(PartMake_GenStores(P.bp3doc_store))
    ...
    Private Function PartMake_FormPanelJS(ByVal ot As bp3doc.bp3doc.bp3doc_def, ByVal P As bp3doc.bp3doc.bp3doc_store, ByVal mode As bp3card.bp3card.Application) As String
        If P.PartType = bp3doc.bp3doc.enumPartType.PartType_Rassirenie Then Return ""
        Dim sw As StringBuilder
        sw = New StringBuilder()

        Dim defmode As bp3card.bp3card.Application = mode
        Dim chkmode As bp3card.bp3card.Application = mode
        ...
        If mode Is Nothing Then
            modename = ""
        Else
            modename = mode.bp3card_def.Item(1).Name
        End If
        ...
        '    If ot.OBJECTMODE.Item(i).DefaultMode = bp3doc.bp3doc.enumBoolean.Boolean_Da Then
        ...
        sw.Append(vbCrLf & "Ext.define('Form_" & P.Name.ToLower() & modename & "', {")
        sw.Append(vbCrLf & "extend:  'Ext.form.Panel',")
        sw.Append(vbCrLf & "alias: 'widget.f_" & P.Name.ToLower() & modename & "',")
        ...
        Dim fg As String
        Dim vIsFieldReadonly As Boolean
        Dim vIsFieldPresent As Boolean
        ...
        For i = 1 To P.bp3doc_field.Count
            fld = P.bp3doc_field.Item(i)
            ft = fld.FieldType
            If IsFieldPresent(P, fld.ID, chkmode) And Not fld.TheStyle.Contains("hidden") Then
        ...
        If HasUnGroupedField Then
            ...
            For i = 1 To P.bp3doc_field.Count
                ...
                If fld.FieldGroupBox = "" Then
                    vIsFieldPresent = IsFieldPresent(P, fld.ID, chkmode) And Not fld.TheStyle.Contains("hidden")
                    If vIsFieldPresent Then

                        vIsFieldReadonly = IsFieldReadonly(P, fld.ID, chkmode)
                        ...
                        sw.Append(vbCrLf & Tool_MakeField(fld, vIsFieldReadonly, mode))
                        sw.Append(vbCrLf & "       ,labelWidth: " + labelWidth.ToString)
                        sw.Append(vbCrLf & "}")
        ...
        For j = 1 To col.Count
            ...
            For i = 1 To P.bp3doc_field.Count
                ...
                If fld.FieldGroupBox = fg Then
                    vIsFieldPresent = IsFieldPresent(P, fld.ID, mode) And Not fld.TheStyle.Contains("hidden")
                    If vIsFieldPresent Then
                        If ft.TypeStyle = bp3doc.bp3doc.enumTypeStyle.TypeStyle_Element_oformleniy And ft.Name = "Panel" Then
                            substThisPanel = True
        ...
        sw.Append(vbCrLf & "Ext.define('EditWindow_" & P.Name.ToLower() & modename & "', {")
        ...
        sw.Append(vbCrLf & "    title:  '" & P.Caption() & "',")
        sw.Append(vbCrLf & "    items:[{")
        sw.Append(vbCrLf & "        xtype:  'f_" & P.Name.ToLower() & modename & "'")
        sw.Append(vbCrLf & "	}]")
        sw.Append(vbCrLf & "	});")
        ...
        'If mode = "" Then
        For i = 1 To P.bp3doc_store.Count
            sw.Append(vbCrLf & PartMake_FormPanelJS(ot, P.bp3doc_store.Item(i), mode))
        Next
        'End If
    ...
    Private Function PartMake_PlaneJS(ByVal ot As bp3doc.bp3doc.bp3doc_def, ByVal P As bp3doc.bp3doc.bp3doc_store, ByVal objmode As bp3card.bp3card.Application, ByVal isMain As Boolean, Optional ByVal TabName As String = "", Optional ByVal tabpostfix As String = "") As String

        Dim sw As StringBuilder
        sw = New StringBuilder()

        Dim mode As bp3card.bp3card.Application = objmode
        Dim modename As String
        If objmode Is Nothing Then
            modename = ""
        Else
        ...
        For i = 1 To P.bp3doc_field.Count
            If IsFieldPresent(P, P.bp3doc_field.Item(i).ID, mode) And Not P.bp3doc_field.Item(i).TheStyle.Contains("hidden") Then
                If P.bp3doc_field.Item(i).TabName & "" = TabName Then
                    If P.bp3doc_field.Item(i).FieldGroupBox <> "" Then
                        fg = P.bp3doc_field.Item(i).FieldGroupBox
                        Try
                            col.Add(fg, fg)
                        Catch ex As Exception
                            ' non unique
                ...
                If P.bp3doc_field.Item(i).TabName & "" = TabName Then
                    If fld.FieldGroupBox = "" Then
                        ...
                        If vIsFieldPresent Then
                            ...
                            vIsFieldReadonly = IsFieldReadonly(P, fld.ID, mode)
        ...
        'If Not false And ot.CommitFullObject = bp3doc.bp3doc.enumBoolean.Boolean_Net Then
        '    sw.Append(vbCrLf & "        ,dockedItems: [{")
        '    sw.Append(vbCrLf & "            xtype:  'toolbar',")
        '    sw.Append(vbCrLf & "            dock:   'bottom',")
        '    sw.Append(vbCrLf & "            ui:     'footer',")
    ...
    Private Function Tool_MakeField(ByVal fld As bp3doc.bp3doc.bp3doc_field, ByVal FldReadOnly As Boolean, ByVal mode As bp3card.bp3card.Application) As String
        ...
        iSMandatory = IsFieldMandatory(fld.Parent.Parent, fld.ID, mode)

        Dim hasInterval As Boolean = False

        If fld.ReferenceType = bp3doc.bp3doc.enumReferenceType.ReferenceType_Skalyrnoe_pole_OPN_ne_ssilkaCLS Then
            ...
            ElseIf ft.TypeStyle = bp3doc.bp3doc.enumTypeStyle.TypeStyle_Perecislenie Then
                ...
                If FldReadOnly Then
                    ...
                    sw.Append(vbCrLf & "hideTrigger: true,")
                Else
                    If Not iSMandatory Then 'fld.AllowNull = bp3doc.bp3doc.enumBoolean.Boolean_Da Then
        ...
        ElseIf fld.ReferenceType = bp3doc.bp3doc.enumReferenceType.ReferenceType_Na_stroku_razdela Then
        ...
        If iSMandatory Then 'fld.AllowNull = bp3doc.bp3doc.enumBoolean.Boolean_Net Then
    ...
    Private Function Tool_MakeFilterField(ByVal fld As bp3list.bp3list.bp3list_filterfield) As String
        ...
        If ft.TypeStyle <> bp3doc.bp3doc.enumTypeStyle.TypeStyle_Ssilka Then
            ...
            ElseIf ft.GridSortType = bp3doc.bp3doc.enumColumnSortType.ColumnSortType_As_String Then
            ...
            ElseIf ft.GridSortType = bp3doc.bp3doc.enumColumnSortType.ColumnSortType_As_Date Then
                ...
                'sw.Append(vbCrLf & "hideLabel:  true,")
                sw.Append(vbCrLf & "listeners: {render: function(e) {Ext.QuickTips.register({  target: e.getEl(), text: '" & fld.Caption & "'});}}")
            ElseIf ft.GridSortType = bp3doc.bp3doc.enumColumnSortType.ColumnSortType_As_Numeric Then
                sw.Append(vbCrLf & "xtype:  'numberfield',")
    ...
    Private Function JournalMake_PHPModel(ByVal Jrnl As bp3list.bp3list.Application) As String
        ...
        Dim part As bp3doc.bp3doc.bp3doc_store
        Dim otype As bp3doc.bp3doc.bp3doc_def
        ...
        otype = TypeForStruct(part)

        Dim fld As bp3doc.bp3doc.bp3doc_field
        Dim ft As bp3ft.bp3ft.bp3ft_def

        For i = 1 To Jrnl.bp3list_col.Count
            For j = 1 To qry.bp3qry_column.Count
                If qry.bp3qry_column.Item(j).the_Alias.ToLower() = Jrnl.bp3list_col.Item(i).ViewField.ToLower() Then
                    fld = qry.bp3qry_column.Item(j).Field
                    ft = fld.FieldType
                    If ft.GridSortType = bp3doc.bp3doc.enumColumnSortType.ColumnSortType_As_Date Then

                        If ft.Name.ToLower = "date" Then
                            flist = flist & ",DATE_FORMAT(" & Jrnl.bp3list_col.Item(i).ViewField.ToLower & ",\'%Y-%m-%d\') " & Jrnl.bp3list_col.Item(i).ViewField.ToLower
                        ElseIf ft.Name.ToLower = "time" Then
                            flist = flist & ",DATE_FORMAT(" & Jrnl.bp3list_col.Item(i).ViewField.ToLower & ",\' %H:%i:%s\') " & Jrnl.bp3list_col.Item(i).ViewField.ToLower
                        ElseIf ft.Name.ToLower = "datetime" Then
                            flist = flist & ",DATE_FORMAT(" & Jrnl.bp3list_col.Item(i).ViewField.ToLower & ",\'%Y-%m-%d %H:%i:%s\') " & Jrnl.bp3list_col.Item(i).ViewField.ToLower
                        ElseIf ft.Name.ToLower = "birthday" Then
                            flist = flist & "," & Jrnl.bp3list_col.Item(i).ViewField.ToLower & " "
                        End If
        ...
        If otype.UseArchiving = bp3doc.bp3doc.enumBoolean.Boolean_Da Then
        ...
        favstr = ""
        'If Jrnl.bp3list_def.Item(1).UseFavorites = bp3list.bp3list.enumBoolean.Boolean_Da Then
        '    favstr = ", 'favorites'=>1"
        'End If
        For i = 1 To Jrnl.bp3list_col.Count
            For j = 1 To qry.bp3qry_column.Count
                If qry.bp3qry_column.Item(j).the_Alias.ToLower() = Jrnl.bp3list_col.Item(i).ViewField.ToLower() Then
                    ...
                    ft = fld.FieldType
                    If ft.GridSortType = bp3doc.bp3doc.enumColumnSortType.ColumnSortType_As_Date Then
                        sw.Append(vbCrLf & "			  case '" & qry.bp3qry_column.Item(j).the_Alias.ToLower() & "_le':")
                        sw.Append(vbCrLf & "			  $cond = '" & qry.bp3qry_column.Item(j).the_Alias.ToLower() & "<=""'.$obj->value.'""';")
                        sw.Append(vbCrLf & "			  break;")
                        sw.Append(vbCrLf & "			  case '" & qry.bp3qry_column.Item(j).the_Alias.ToLower() & "_ge':")
                        sw.Append(vbCrLf & "			  $cond = '" & qry.bp3qry_column.Item(j).the_Alias.ToLower() & ">=""'.$obj->value.'""';")
                        sw.Append(vbCrLf & "			  break;")
                    End If
                    If ft.GridSortType = bp3doc.bp3doc.enumColumnSortType.ColumnSortType_As_Numeric Then
        ...
        If otype.UseArchiving = bp3doc.bp3doc.enumBoolean.Boolean_Net Then
    ...
    Private Function JournalMake_PHPController(ByVal Jrnl As bp3list.bp3list.Application) As String
    ...
    Private Function JournalMake_JSModel(ByVal Jrnl As bp3list.bp3list.Application) As String
        Dim sw As StringBuilder
        sw = New StringBuilder
        ...
        For i = 1 To Jrnl.bp3list_col.Count
            For j = 1 To qry.bp3qry_column.Count
                If qry.bp3qry_column.Item(j).the_Alias.ToLower() = Jrnl.bp3list_col.Item(i).ViewField.ToLower() Then
                    ...
                    ft = fld.FieldType
                    If ft.GridSortType = bp3doc.bp3doc.enumColumnSortType.ColumnSortType_As_Date Then
                        sw.Append(vbCrLf & "            ,{name:'" & Jrnl.bp3list_col.Item(i).ViewField.ToLower() & "', type: 'string'}")
                    End If
                    If ft.GridSortType = bp3doc.bp3doc.enumColumnSortType.ColumnSortType_As_Numeric Then
                        sw.Append(vbCrLf & "            ,{name:'" & Jrnl.bp3list_col.Item(i).ViewField.ToLower() & "', type: 'number'}")
                    End If
                    If ft.GridSortType = bp3doc.bp3doc.enumColumnSortType.ColumnSortType_As_String Then
                        sw.Append(vbCrLf & "            ,{name:'" & Jrnl.bp3list_col.Item(i).ViewField.ToLower() & "', type: 'string'}")
                    End If
    ...
    Private Function JournalMake_JSStore(ByVal Jrnl As bp3list.bp3list.Application) As String
        Dim sw As StringBuilder
        sw = New StringBuilder
        Dim qry As bp3qry.bp3qry.Application
        Dim pv As bp3qry.bp3qry.bp3qry_def
        ...
        part = pv.SrcPart
        doc_app = part.Application
        otype = doc_app.bp3doc_def.Item(1)
    ...
    Private Function JournalMake_Filter(ByVal fltr As bp3list.bp3list.Application) As String
        ...
        Dim sw As StringBuilder
        sw = New StringBuilder
        If fltr.bp3list_filter.Count > 0 Then
            ...
            Dim fld As bp3list.bp3list.bp3list_filterfield
            Dim ft As bp3ft.bp3ft.bp3ft_def
            Dim j As Integer
            Dim FirstField As Boolean = True
            fltr.bp3list_filter.Sort = "sequence"
            For i = 1 To fltr.bp3list_filter.Count
                fg = fltr.bp3list_filter.Item(i)
                fg.bp3list_filterfield.Sort = "sequence"
                For j = 1 To fg.bp3list_filterfield.Count
                    fld = fg.bp3list_filterfield.Item(j)
                    ft = CType(fld.FieldType, bp3ft.bp3ft.bp3ft_def)
            ...
            'If Jrnl.bp3list_def.Item(1).UseFavorites = MTZJrnl.MTZJrnl.enumBoolean.Boolean_Da Then
    ...
    Private Function JournalMake_JSGrid(ByVal Jrnl As bp3list.bp3list.Application) As String
        Dim sw As StringBuilder
        sw = New StringBuilder
        ...
        Try
            ...
            saveall = ot.CommitFullObject = bp3doc.bp3doc.enumBoolean.Boolean_Da
        Catch ex As Exception
            ot = Nothing
            saveall = False
        End Try
        ...
        Jrnl.bp3list_col.Sort = "sequence"

        Dim jc As bp3list.bp3list.bp3list_col
        Dim wi As Integer
        For i = 1 To Jrnl.bp3list_col.Count
            jc = Jrnl.bp3list_col.Item(i)


            If i > 1 Then
                sw.Append(vbCrLf & "            ,")
            End If
            If Jrnl.bp3list_col.Count = 1 Then
                wi = 800
            Else
                wi = 800 / Jrnl.bp3list_col.Count
                If wi < 120 Then
                    wi = 120
                End If
            End If
            ...
            fld = Nothing
            ft = Nothing
            For ii = 1 To qry.bp3qry_column.Count

                If qry.bp3qry_column.Item(ii).the_Alias.ToLower() = jc.ViewField.ToLower() Then
                    fld = qry.bp3qry_column.Item(ii).Field
                    ft = fld.FieldType
                    Exit For
                End If
            ...
            If Not ft Is Nothing Then
                If ft.Name.ToLower = "file" Then
                    ...
                    sw.Append(vbCrLf & "tpl:'<a href=\'/output_file.php?ID={" & jc.ViewField.ToLower() & "}&ext={" & jc.ViewField.ToLower() & "_ext}\' target=\'_blank\'>Файл</a>'}")
                ElseIf ft.Name.ToLower = "url" Then
                    sw.Append(vbCrLf & "{ text     : '" & jc.name & "', xtype: 'templatecolumn',  align:'right',width    : 90,	sortable : false,")
                    sw.Append(vbCrLf & "tpl:'<a href=\'{" & jc.ViewField.ToLower() & "}\' target=\'_blank\'>" & jc.name & "</a>'}")
                ElseIf ft.Name.ToLower = "html" Then
                    sw.Append(vbCrLf & "{text: """ & jc.name & """, width: 200, dataIndex: '" & jc.ViewField.ToLower() & "', sortable: true,")
                    sw.Append(vbCrLf & " renderer: function(value){var S =new String(value);  S=S.replace(new RegExp('/>','g'),'');  S=S.replace(new RegExp('<','g'),''); S=S.replace(new RegExp('>','g'),''); if(S.length >255) S=S.substr(0,255); return S;}}")

                Else
                    If ft.GridSortType = bp3doc.bp3doc.enumColumnSortType.ColumnSortType_As_Date Then
                        sw.Append(vbCrLf & "            {text: """ & jc.name & """, width:" & wi.ToString & ", dataIndex: '" & jc.ViewField.ToLower() & "', sortable: true,renderer:myDateRenderer}")
                    Else
                        sw.Append(vbCrLf & "            {text: """ & jc.name & """, width:" & wi.ToString & ", dataIndex: '" & jc.ViewField.ToLower() & "', sortable: true}")
                    End If
        ...
        If Not ot Is Nothing Then
            sw.Append(vbCrLf & "                edit.setTitle('Создание документа:" + ot.TheCaption + "') ;")
        Else
        ...
        sw.Append(vbCrLf & "                edit.prefix='c_v_" + pv.the_Alias.ToLower() + "';")
        If Not ot Is Nothing Then
            sw.Append(vbCrLf & "                edit.setTitle('Редактирование документа: " + ot.TheCaption + "') ;")
        Else
        ...
        sw.Append(vbCrLf & "       title: '" & ot.TheCaption & "',")
    ...
    Private Sub cmdGen_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdGen.Click
        ...
        For i = 0 To dt_ot.Rows.Count - 1
            doc_app = Manager.GetInstanceObject(New Guid(dt_ot.Rows(i)("instanceid").ToString()))
            ot = doc_app.bp3doc_def.Item(1)
            sw.Append(PartMake_GenStores(doc_app.bp3doc_store))
            pb.Value = i
            Application.DoEvents()
        Next
        ...
        For i = 0 To chkObjType.CheckedItems.Count - 1
            ...
            Dim crd_app As bp3card.bp3card.Application
            For j = 0 To dt_crd.Rows.Count - 1
                crd_app = Manager.GetInstanceObject(New Guid(dt_crd.Rows(j)("instanceid").ToString()))
                TypeMaker(doc_app, crd_app)
            Next
            TypeMaker(doc_app, Nothing)
            ti = Nothing
        Next

...
Public Class frmB2SercherMYSQL
    ...
    Private Sub GenTypeJS(ByVal ID As Guid)
        ...
        mytemplate = mytemplate.Replace("%SW_TOWN%", GetTownJS())
    ...
    Private Function GetDistrictJS() As String
        ...
        For i = 0 To reg.Rows.Count - 1
            ...
            dist = Manager.Session.GetData("select treecode,name from b2g_addr where objecttype in (2,8) and  treecode like '" + reg.Rows(i)("treecode") + "______' order by name")
    ...
    Private Function GetSubwayJS() As String
        ...
        For i = 0 To reg.Rows.Count - 1
            ...
            dist = Manager.Session.GetData("select TSubwayid,TSubway.name from TSubway join b2g_addr on tsubway.name=b2g_addr.name  where objecttype in (5,6) and  treecode like '" + reg.Rows(i)("treecode") + "______' order by name")
            For j = 0 To dist.Rows.Count - 1
    ...
    Private Function GetTownJS() As String
        Dim sw As StringBuilder
        sw = New StringBuilder
        Dim reg As DataTable
        Dim dist As DataTable
        Dim i As Integer
        Dim j As Integer
        reg = Manager.Session.GetData("select treecode,name from b2g_addr where objecttype in (2,8) and datalength(treecode)=12 order by name")
        For i = 0 To reg.Rows.Count - 1
            sw.Append(vbCrLf & "if (id.value == '" + reg.Rows(i)("treecode") + "') {")
            sw.Append(vbCrLf & "       addOpt(cbox, '(Выбрать нас. пункт)', '-1');")
            dist = Manager.Session.GetData("select treecode,name from b2g_addr  where objecttype in (3) and datalength(treecode)=18 and  treecode like '" + reg.Rows(i)("treecode") + "%' order by name")
            For j = 0 To dist.Rows.Count - 1
                sw.Append(vbCrLf & "      addOpt(cbox, '" + dist.Rows(j)("name") + "', '" + dist.Rows(j)("treecode").ToString + "');")
            Next
            sw.Append(vbCrLf & "}")
        Next
    ...
    Private Function MakeJPHPModel(ByVal ID As Guid) As String
        ...
        flist = "v_" + pv.the_Alias + ".instanceid,id"
        ...
        For i = 1 To Jrnl.JournalColumn.Count
            ...
            If Jrnl.JournalColumn.Item(i).ColSort = MTZJrnl.MTZJrnl.enumColumnSortType.ColumnSortType_As_Date Then
                flist = flist & ",DATE_FORMAT(" & Jrnl.JournalColumn.Item(i).JColumnSource.Item(1).ViewField.ToLower & ",\'%d/%m/%Y %H:%i:%s\') " & Jrnl.JournalColumn.Item(i).JColumnSource.Item(1).ViewField.ToLower
            Else
        ...
        sw.Append(vbCrLf & "	    $sql='select  " & flist & " from v_" & pv.the_Alias.ToLower() & "'.$w;")
        sw.Append(vbCrLf & "	   //mysql_select_db('b2');")
        sw.Append(vbCrLf & "	    $stmt= mysql_query(  $sql);")
        sw.Append(vbCrLf & "        $result=array();")
        sw.Append(vbCrLf & "	    if($row=mysql_fetch_assoc( $stmt)){")
        sw.Append(vbCrLf & "		    $result[]=$row;")
        sw.Append(vbCrLf & "	    }")
        sw.Append(vbCrLf & "	    mysql_free_result( $stmt);")
        ...
        Select Case ot.Name.ToUpper
            ...
            Case "B2P"
                ...
                sw.Append(vbCrLf & "        			$w=$w."" and ( v_auto" + part.Name.ToLower() + ".instanceid in (select distinct instanceid from b2p_photo))  "";")
                sw.Append(vbCrLf & "        			break;")

            Case "B2S"
                sw.Append(vbCrLf & "        			case 'OnlyPhoto':")
                sw.Append(vbCrLf & "        			$w=$w."" and ( v_auto" + part.Name.ToLower() + ".instanceid in (select distinct instanceid from b2s_photo))  "";")
                sw.Append(vbCrLf & "        			break;")

            Case "B2Z"
                sw.Append(vbCrLf & "        			case 'OnlyPhoto':")
                sw.Append(vbCrLf & "        			$w=$w."" and ( v_auto" + part.Name.ToLower() + ".instanceid in (select distinct instanceid from bz_photo))  "";")
                sw.Append(vbCrLf & "        			break;")

            Case "B2LR"
                sw.Append(vbCrLf & "        			case 'OnlyPhoto':")
                sw.Append(vbCrLf & "        			$w=$w."" and ( v_auto" + part.Name.ToLower() + ".instanceid in (select distinct instanceid from b2lr_photo))  "";")
                sw.Append(vbCrLf & "        			break;")

            Case "B2FR"
                sw.Append(vbCrLf & "        			case 'OnlyPhoto':")
                sw.Append(vbCrLf & "        			$w=$w."" and ( v_auto" + part.Name.ToLower() + ".instanceid in (select distinct instanceid from b2fr_photo))  "";")
                sw.Append(vbCrLf & "        			break;")
            Case "B2C"
                sw.Append(vbCrLf & "        			case 'OnlyPhoto':")
                sw.Append(vbCrLf & "        			$w=$w."" and ( v_auto" + part.Name.ToLower() + ".instanceid in (select distinct instanceid from b2c_photo))  "";")
                sw.Append(vbCrLf & "        			break;")
                ...
                If pv.ViewColumn.Item(j).the_Alias = Jrnl.JournalColumn.Item(i).JColumnSource.Item(1).ViewField Then
                    ...
                    If ft.TypeStyle = MTZMetaModel.MTZMetaModel.enumTypeStyle.TypeStyle_Skalyrniy_tip Then
                        ...
                        If Jrnl.JournalColumn.Item(i).ColSort = MTZJrnl.MTZJrnl.enumColumnSortType.ColumnSortType_As_Date Then
                            ...
                            sw.Append(vbCrLf & "        		    $w=$w."" and DATE_FORMAT(" + pv.ViewColumn.Item(j).the_Alias + ",'%d/%m/%Y %H:%i:%s') = DATE_FORMAT('"".$p[1].""','%d/%m/%Y %H:%i:%s')"";")
        ...
        sw.Append(vbCrLf & "	$sql1='select count(*)  total  from v_" & pv.the_Alias.ToLower() & "'.$w;")
        'sw.Append(vbCrLf & "	$sql='select  " & flist & " from v_" & pv.the_Alias.ToLower() & "'.$w;")

        Select Case ot.Name.ToUpper
            Case "B2P"
                sw.Append(vbCrLf & "	$sql='select " & flist & ",IFNULL(b2p_photo.filename,\'\') filename from v_" & pv.the_Alias.ToLower() & " left join b2p_photo on v_" & pv.the_Alias.ToLower() & ".instanceid=b2p_photo.instanceid and b2p_photo.ismain=-1 '.$w;")
            Case "B2S"
                sw.Append(vbCrLf & "	$sql='select " & flist & ",IFNULL(b2s_photo.filename,\'\') filename from v_" & pv.the_Alias.ToLower() & " left join b2s_photo on v_" & pv.the_Alias.ToLower() & ".instanceid=b2s_photo.instanceid and b2s_photo.ismain=-1 '.$w;")
            Case "B2Z"
                sw.Append(vbCrLf & "	$sql='select " & flist & ",IFNULL(b2z_photo.filename,\'\') filename from v_" & pv.the_Alias.ToLower() & " left join b2z_photo on v_" & pv.the_Alias.ToLower() & ".instanceid=b2z_photo.instanceid and b2z_photo.ismain=-1 '.$w;")
            Case "B2LR"
                sw.Append(vbCrLf & "	$sql='select " & flist & ",IFNULL(b2lr_photo.filename,\'\') filename from v_" & pv.the_Alias.ToLower() & " left join b2lr_photo on v_" & pv.the_Alias.ToLower() & ".instanceid=b2lr_photo.instanceid and b2lr_photo.ismain=-1 '.$w;")
            Case "B2FR"
                sw.Append(vbCrLf & "	$sql='select " & flist & ",IFNULL(b2fr_photo.filename,\'\') filename from v_" & pv.the_Alias.ToLower() & " left join b2fr_photo on v_" & pv.the_Alias.ToLower() & ".instanceid=b2fr_photo.instanceid and b2fr_photo.ismain=-1 '.$w;")
            Case "B2C"
                sw.Append(vbCrLf & "	$sql='select " & flist & ",IFNULL(b2c_photo.filename,\'\') filename from v_" & pv.the_Alias.ToLower() & " left join b2c_photo on v_" & pv.the_Alias.ToLower() & ".instanceid=b2c_photo.instanceid and b2c_photo.ismain=-1 '.$w;")
        ...
        'sw.Append(vbCrLf & "	$cn=$this->db2->db_connect();")
        sw.Append(vbCrLf & "	//mysql_select_db('b2');")
        sw.Append(vbCrLf & "	$stmt= mysql_query(  $sql1);")
        ...
        sw.Append(vbCrLf & "	$stmt= mysql_query( $sql.' LIMIT '.$offset.','.$limit);")
        sw.Append(vbCrLf & "	$result=array();")
        sw.Append(vbCrLf & "		while( $row = mysql_fetch_assoc( $stmt))")
        sw.Append(vbCrLf & "		{")
        sw.Append(vbCrLf & "			$result[]=$row;")
        sw.Append(vbCrLf & "		}")
        sw.Append(vbCrLf & "	mysql_free_result( $stmt);")
        ...
        If ot.Name.ToUpper = "B2S" Or ot.Name.ToUpper = "B2Z" Or ot.Name.ToUpper = "B2C" Or ot.Name.ToUpper = "B2FR" Or ot.Name.ToUpper = "B2P" Or ot.Name.ToUpper = "B2LR" Then
            ...
            sw.Append(vbCrLf & "   $w ="" where " + tt + "_infoid=G2B('"".$ID.""')"";")
            ...
            sw.Append(vbCrLf & "{")
            sw.Append(vbCrLf & "   log_message('debug','getMainPhoto');")
            sw.Append(vbCrLf & "   $w ="" where " + tt + "_photo.ismain=-1 and " + tt + "_infoid=G2B('"".$ID.""')"";")
            sw.Append(vbCrLf & "   $sql='select   " + tt + "_photo.instanceid, " + tt + "_photo." + tt + "_photoid, " + tt + "_photo.filename, " + tt + "_photo.ismain from " + tt + "_photo join " + tt + "_info on " + tt + "_info.instanceid=" + tt + "_photo.instanceid '.$w;")
            sw.Append(vbCrLf & " //mysql_select_db('b2');")
            sw.Append(vbCrLf & "   $stmt= mysql_query( $sql );")
            sw.Append(vbCrLf & "   $result=array();")

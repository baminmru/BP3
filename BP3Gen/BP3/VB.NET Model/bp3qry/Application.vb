...
''' <summary>
''' Внутренняя переменная для раздела Колонка
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_bp3qry_column As bp3qry_column_col


''' <summary>
'''Свойство для доступа к строкам раздела Колонка
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property bp3qry_column() As bp3qry_column_col
            Get
                If m_bp3qry_column Is Nothing Then
                    m_bp3qry_column = New bp3qry_column_col
                    m_bp3qry_column.Application = Me
                    m_bp3qry_column.Parent = Me
                    m_bp3qry_column.Refresh()
                End If
                bp3qry_column = m_bp3qry_column
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Представление
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_bp3qry_def As bp3qry_def_col


''' <summary>
'''Свойство для доступа к строкам раздела Представление
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property bp3qry_def() As bp3qry_def_col
            Get
                If m_bp3qry_def Is Nothing Then
                    m_bp3qry_def = New bp3qry_def_col
                    m_bp3qry_def.Application = Me
                    m_bp3qry_def.Parent = Me
                    m_bp3qry_def.Refresh()
                End If
                bp3qry_def = m_bp3qry_def
            End Get
        End Property


''' <summary>
''' Внутренняя переменная для раздела Связанные представления
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_bp3qry_link As bp3qry_link_col


''' <summary>
'''Свойство для доступа к строкам раздела Связанные представления
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property bp3qry_link() As bp3qry_link_col
            Get
                If m_bp3qry_link Is Nothing Then
                    m_bp3qry_link = New bp3qry_link_col
                    m_bp3qry_link.Application = Me
                    m_bp3qry_link.Parent = Me
                    m_bp3qry_link.Refresh()
                End If
                bp3qry_link = m_bp3qry_link
            End Get
        End Property
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = bp3qry_def.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = bp3qry_column.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = bp3qry_link.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
            ...
            e_list = node.SelectNodes("bp3qry_def_COL")
            bp3qry_def.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("bp3qry_column_COL")
            bp3qry_column.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("bp3qry_link_COL")
            bp3qry_link.XMLLoad(e_list, LoadMode)
        End Sub
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            bp3qry_def.XMLSave(node, Xdom)
            bp3qry_column.XMLSave(node, Xdom)
            bp3qry_link.XMLSave(node, Xdom)
        End Sub
...
Public Overrides Sub BatchUpdate()
    bp3qry_def.BatchUpdate
    bp3qry_column.BatchUpdate
    bp3qry_link.BatchUpdate
End Sub
    End Class
End Namespace

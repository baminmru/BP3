...
''' <summary>
''' Внутренняя переменная для раздела Ограничение уникальности
''' </summary>
''' <remarks>
'''
''' </remarks>
  Private m_bp3doc_uk As bp3doc_uk_col


''' <summary>
'''Свойство для доступа к строкам раздела Ограничение уникальности
''' </summary>
''' <remarks>
'''
''' </remarks>
        Public ReadOnly Property bp3doc_uk() As bp3doc_uk_col
            Get
                If m_bp3doc_uk Is Nothing Then
                    m_bp3doc_uk = New bp3doc_uk_col
                    m_bp3doc_uk.Application = Me
                    m_bp3doc_uk.Parent = Me
                    m_bp3doc_uk.Refresh()
                End If
                bp3doc_uk = m_bp3doc_uk
            End Get
        End Property
...
'''
''' </remarks>
  Private m_bp3doc_def As bp3doc_def_col
...
'''
''' </remarks>
        Public ReadOnly Property bp3doc_def() As bp3doc_def_col
            Get
                If m_bp3doc_def Is Nothing Then
                    m_bp3doc_def = New bp3doc_def_col
                    m_bp3doc_def.Application = Me
                    m_bp3doc_def.Parent = Me
                    m_bp3doc_def.Refresh()
                End If
                bp3doc_def = m_bp3doc_def
            End Get
        End Property
...
''' </remarks>
        ...
        FindInCollections = Nothing
            ...
            mFindInCollections = bp3doc_def.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = bp3doc_store.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
            mFindInCollections = bp3doc_uk.FindObject(Table, InstID)
            if not mFindInCollections is nothing then return mFindInCollections
        End Function
...
''' </remarks>
        Protected Overrides Sub XMLLoadCollections(ByVal node As System.Xml.XmlNode, Optional ByVal LoadMode As Integer = 0)
            ...
            e_list = node.SelectNodes("bp3doc_def_COL")
            bp3doc_def.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("bp3doc_store_COL")
            bp3doc_store.XMLLoad(e_list, LoadMode)
            e_list = node.SelectNodes("bp3doc_uk_COL")
            bp3doc_uk.XMLLoad(e_list, LoadMode)
        End Sub
...
''' </remarks>
        Public Overrides Sub XMLSaveCollections(ByVal node As System.Xml.XmlElement, ByVal Xdom As System.Xml.XmlDocument)
            bp3doc_def.XMLSave(node, Xdom)
            bp3doc_store.XMLSave(node, Xdom)
            bp3doc_uk.XMLSave(node, Xdom)
        End Sub
...
Public Overrides Sub BatchUpdate()
    bp3doc_def.BatchUpdate
    bp3doc_store.BatchUpdate
    bp3doc_uk.BatchUpdate
End Sub
    End Class
End Namespace
